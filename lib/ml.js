...
export function getTrends(currentLog, history) {
  ...
  if (currentLog.workouts > (prev.workouts || 0))
    trends.push({ good: true, text: `+${currentLog.workouts - prev.workouts} more workouts than yesterday` });
  else if (currentLog.workouts < (prev.workouts || 0))
    trends.push({ good: false, text: `${(prev.workouts||0) - currentLog.workouts} fewer workouts than yesterday` });
  if (currentLog.ateOut < (prev.ateOut || 0))
    trends.push({ good: true, text: `Ate out ${(prev.ateOut||0) - currentLog.ateOut}x less than yesterday` });
  else if (currentLog.ateOut > (prev.ateOut || 0))
    trends.push({ good: false, text: `Ate out ${currentLog.ateOut - (prev.ateOut||0)}x more than yesterday` });
  if (currentLog.sleep >= 8 && (prev.sleep || 0) < 8)
    trends.push({ good: true, text: "First day hitting 8h sleep 🌙" });
...
export function generateShoutout(userName, currentLog, history) {
  ...
  if (currentLog.workouts > prevBest("workouts") && currentLog.workouts >= 4)
    shoutouts.push(`🔥 ${userName} just hit their most active day ever — ${currentLog.workouts} workouts logged. Beast mode confirmed.`);
  else if (currentLog.workouts >= avg.workouts * 1.5 && currentLog.workouts >= 3)
    shoutouts.push(`💪 ${userName} is on a workout tear — ${currentLog.workouts} sessions today.`);

  if (currentLog.ateOut === 0)
    shoutouts.push(`👨‍🍳 ${userName} cooked every single meal today. Zero restaurants. Absolutely unhinged discipline.`);
  else if (currentLog.ateOut > prevBest("ateOut") && currentLog.ateOut >= 5)
    shoutouts.push(`💸 ${userName} just set a new eat-out record — ${currentLog.ateOut} times today 💀`);

  if (currentLog.junk === 0 && avg.junk > 1)
    shoutouts.push(`🥗 ${userName} went completely junk-free today. Coming off an avg of ${Math.round(avg.junk)} junk meals — that's a glow up.`);

  if (currentLog.sugar === 0)
    shoutouts.push(`🍃 ${userName} had zero sugar meals today. The discipline is genuinely alarming.`);

  if (currentLog.sleep >= 8 && avg.workouts >= 3)
    shoutouts.push(`⚡ ${userName} is sleeping 8h AND working out ${currentLog.workouts}x today. They've cracked the code.`);

  if (!shoutouts.length)
    shoutouts.push(`✅ ${userName} logged today. Balance score: ${getBalanceScore(currentLog)}/100. Showing up matters.`);
