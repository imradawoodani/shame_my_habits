...
export default async function handler(req, res) {
  ...
  const fallback = `This week, ${userName || "you"} showed up. Not perfect — but consistent. ${archetype?.emoji || ""} The best habits are built in moments like this one.`;

  if (!process.env.OPENAI_API_KEY) {
    return res.json({ narrative: fallback });
  }

  const prompt = `Write a Spotify Wrapped-style personal narrative for ${userName}'s weekly habit data.
