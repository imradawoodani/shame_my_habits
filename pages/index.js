...
import {
  ...
  getCurrentLog, saveCurrentLog, getHistory, commitWeek,
  getFeed, saveFeed,
} from "../lib/store";
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", gap: 24, color: "var(--muted)", fontSize: 12 }}>
        {["📊 Balance Score", "🏆 Leaderboard", "🎧 Weekly Wrapped"].map(f => (
          <span key={f}>{f}</span>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{ padding: "24px 16px 100px", display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ fontSize: 13, color: "var(--muted)", marginTop: 4 }}>
            {score >= 70 ? "You're crushing it" : score >= 50 ? "Solid week" : "Room to improve"}
          </p>
        </div>
      ...
      <div style={{
        ...
        <Counter label="Workouts" emoji="💪" value={log.workouts} onChange={v => { const l = {...log, workouts: v}; setLog(l); saveCurrentLog(l); }} max={7} />
        <Counter label="Sleep avg" emoji="😴" value={log.sleep} onChange={v => { const l = {...log, sleep: v}; setLog(l); saveCurrentLog(l); }} max={12} unit="hrs" />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {history.slice(0, 4).map((h, i) => (
              <div key={i} style={{
                ...
                <div>
                  <p style={{ fontSize: 13, fontFamily: "Syne", fontWeight: 600 }}>{h.week || `Week ${history.length - i}`}</p>
...
// ─── CIRCLE TAB ──────────────────────────────────────────────────────────────
function CircleTab({ feed, setFeed, myLog, user }) {
  const [replyInputs, setReplyInputs] = useState({});
  const [showLeaderboard, setShowLeaderboard] = useState(true);

  const allUsers = [
    { name: user.name, initials: user.initials || user.name[0], log: myLog },
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {feed.length === 0 && (
          <div style={{
            ...
            No posts yet. Log your week to kick things off. 👆
          </div>
        )}
        {feed.map((post) => (
...
function WrappedTab({ myLog, user, history }) {
  ...
  const start = async () => {
    ...
    try {
      ...
      setNarrative(`This week, ${user.name} showed up. Not perfect — but consistent. The best habits are built in moments like this.`);
    }
    setLoading(false);
  };
  ...
  const slides = [
    ...
    { bg: "var(--bg)", content: (
      <div style={{ padding: "40px 24px", width: "100%" }}>
        <p style={{ fontSize: 11, letterSpacing: 3, color: "var(--muted)", fontFamily: "Syne", marginBottom: 24 }}>THIS WEEK</p>
  ...
  if (!started) {
    return (
      <div style={{ padding: "24px 16px 100px", display: "flex", flexDirection: "column", gap: 20 }}>
        <div>
          <p style={{ color: "var(--muted)", fontSize: 12, fontFamily: "Syne", letterSpacing: 2 }}>WEEKLY REVEAL</p>
...
export default function App() {
  ...
  return (
    <div style={{ background: "var(--bg)", minHeight: "100vh" }}>
      {tab === "hub" && <HubTab user={user} log={log} setLog={setLog} history={history} onLog={handleLog} />}
      {tab === "circle" && <CircleTab feed={feed} setFeed={setFeed} myLog={log} user={user} />}
